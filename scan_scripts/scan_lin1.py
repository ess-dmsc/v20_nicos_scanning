...
SCAN_NAME = "lin1"
POSITIONS_1 = np.arange(190, 170, -0.5)
NICOS_MOTOR_1 = "lin1"
...
def do_scan():
    ...
    for i in POSITIONS_1:
        ...
        outputs.append((position, next_value - last_value))
    ...
    file.write("\t".join(["Position", "Counts", "Error"]) + "\n")
    for position, count in outputs:
        file.write("%.3f\t%d\t%.3f\n" % (position, count, sqrt(count)))

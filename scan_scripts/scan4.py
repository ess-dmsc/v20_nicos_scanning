...
SCAN_NAME = "slit3_horizontal"
POSITIONS_1 = np.arange(-2.5, 15, 0.5)
#POSITIONS_2 = [0, 1, 2, 3, 4, 5]
NICOS_MOTOR_1 = "slit3h_center"
...
def do_scan():
    ...
    for i in POSITIONS_1:
        ...
        maw(NICOS_MOTOR_1, i)
        #maw(NICOS_MOTOR_1, (-11.7, i, 15, 0.2))
        ...
        centerx = slit3h_center.read()
        centery = slit3v_center.read()
        width = slit3h_gap.read()
        height = slit3v_gap.read()
    ...
    file = open("/hzb/caress/mike/v20_nicos_scanning/scans/%s_%s.dat" % (SCAN_NAME, timestamp), "w")
